 ...
 */
$render_month = static function ( array $month, &$flagged_next ) {
	?>
	...
	<div class="wcal-timeline">
		<?php foreach ( $month['events'] as $event ) : ?>
			<div class="wcal-row">
				...
				<div class="wcal-body">
					<?php if ( ! $flagged_next ) : ?>
						<span class="wcal-next"><?php esc_html_e( 'Next Mass', 'wp-calendar-plugin' ); ?></span>
...
<div class="wcal-schedule">
	...
	<?php if ( empty( $months ) ) : ?>
		...
		<p class="wcal-empty">
			<?php esc_html_e( 'No upcoming Mass dates are scheduled right now — please check back soon.', 'wp-calendar-plugin' ); ?>
		</p>
			...
			if ( $index === $initial_months && $hidden_event_count > 0 ) :
				...
				<details class="wcal-more">
					<summary class="wcal-more-toggle">
						...
						echo esc_html(
							sprintf(
								/* translators: %d: number of additional Mass dates tucked behind the toggle */
								__( 'Show %d more Masses', 'wp-calendar-plugin' ),
								$hidden_event_count
							)
						);
						?>
